...
// No more ACTION_WORDS, we will prompt scrolling instead.
...
export default async function HomePage() {
    ...
    const reviews: Review[] = JSON.parse(
        await fs.readFile(reviewsPath, "utf-8"),
    );

    const topDrinks = [...reviews]
    ...
    return (
        <div className="relative">
            ...
            <section className="relative z-10 container mx-auto px-6 py-20 md:py-28 flex flex-col items-center justify-center text-center">
                ...
                {/* Speed lines behind the hero panel */}
                <svg
                    className="absolute inset-0 w-full h-full pointer-events-none speed-lines"
                ...
                <div className="bg-[#00fbfb] comic-border comic-shadow-lg p-8 md:p-14 -rotate-1 relative max-w-3xl w-full hero-animate" style={{ zIndex: 2 }}>
                    {/* Starburst top-right — pops in after hero */}
                    <div className="absolute -top-8 -right-8 z-10 star-pop" style={{ '--delay': '0.35s' } as React.CSSProperties}>
                        <StarburstBadge
                            value="38"
                            subLabel="DRINKS!"
                            size="lg"
                    ...
                    <div className="absolute -bottom-6 -left-6 z-10 star-pop-neg" style={{ '--delay': '0.5s' } as React.CSSProperties}>
                        <StarburstBadge
                            value={`${avgRating}/10`}
                            subLabel="AVG"
                            color="#fe00fe"
                    ...
                    <p className="font-vietnam text-lg md:text-xl text-[#3a4a49] max-w-xl mx-auto mb-8 bg-white p-4 comic-border comic-shadow-sm">
                        38 energy drinks. All tried, all rated, all brutally
                        honest. Now with a{" "}
                        <strong>machine learning model </strong> that predicts
                        what I&apos;d think before I even crack one open.
                    </p>
            ...
            <section className="relative z-10 py-16 overflow-hidden">
                <div className="container mx-auto px-6">
                    <h2 className="font-bangers text-4xl md:text-5xl mb-10 text-center -rotate-1 tracking-wider section-reveal" style={{ '--delay': '100ms' } as React.CSSProperties}>
                        <span className="bg-[#e8e8e8] px-4 py-2 comic-border comic-shadow inline-block">
                            THE SHOWCASE OF CHAMPIONS
                        </span>
                    </h2>
                </div>
                <div className="flex auto-scroll-container w-max">
                    {/* Auto-scroll CSS handles the continuous movement */}
                    {[1, 2, 3].map(group => (
                        <div key={group} className="flex gap-6 pr-6">
                            {topDrinks.map((drink, idx) => (
                                <Link
                                    ...
                                    className="shrink-0 w-64 md:w-72 relative group flex flex-col card-stagger"
                                    style={{ '--delay': `${idx * 60}ms` } as React.CSSProperties}
                                >
                                    {/* Speech bubble — appears on hover */}
                            <div
                                className="absolute -top-2 left-1/2 -translate-x-1/2 w-56 opacity-0 group-hover:opacity-100 group-hover:-translate-y-2 transition-all duration-300 z-20 pointer-events-none"
                                style={{
                                    bottom: "calc(100% - 8px)",
                                    top: "auto",
                                }}
                            ...
                            {/* Brand tag */}
                            <div
                                className="font-bangers text-sm tracking-widest px-3 py-1 comic-border inline-block self-start"
                                style={{
                                    background: getBrandColor(drink.brand),
                                }}
                            >
                                {drink.brand}
                            </div>
            ...
            <section className="relative z-10 container mx-auto px-6 py-12 section-reveal" style={{ '--delay': '200ms' } as React.CSSProperties}>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-5 gap-4">
                    {brandStats.map(({ brand, avg, count }, i) => (
                        <div
                            key={brand}
                            className="halftone-bg comic-border comic-shadow p-4 flex flex-col items-center text-center relative group hover:-translate-y-2 hover:translate-x-1 transition-transform duration-200 card-stagger"
                            style={{ '--delay': `${i * 50}ms` } as React.CSSProperties}
                        >
                            <div className="font-bangers text-lg tracking-wider mb-2 leading-tight">
                                {brand}
                            </div>
                            <div className="font-bangers text-3xl text-[#006a6a] mb-1">
                                {avg.toFixed(1)}
                            </div>
                            <div className="font-vietnam text-xs text-[#6a7a7a]">
                ...
                <div className="container mx-auto flex flex-col md:flex-row items-center justify-between gap-8">
                    <div>
                        ...
                        <p className="font-vietnam text-[#b9cac9] text-lg">
                            Pick any drink profile — tried or untried. The Ridge
                            Regression model will predict the rating.
                        </p>
                    </div>
                    <Link
                        href="/oracle"
                        className="font-bangers text-2xl px-10 py-5 bg-[#fe00fe] text-white comic-border pulse-glow whitespace-nowrap tracking-wider flex-shrink-0"
                    >
                        PREDICT NOW
                    </Link>
                </div>
